...
class EmptyMessage(message: ReceiveMessages) : Handling(message.messageID, message.json) {
    companion object {
        private val emptyJson = JSONObject();
    }

    override fun onRec(): Message {
        return Message(isSuccess = false, "未知的类型，消息跳过处理", emptyJson)
    }
}